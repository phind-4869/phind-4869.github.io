...
categories: [杂记, Cpp]
tags: [c++, c++20, 编程语言]     # TAG names should always be lowercase
---
...
第二种方法是引入 `auto`{:.language-cpp} 关键字，这种方法的好处是可以不用再写一遍泛型：

```cpp
Addable auto add(Addable auto a, Addable auto b);
```

第三种方法是在模板定义中，使用概念名来替代 `typename`{:.language-cpp} 关键字：
...
这种写法相比于第一种写法更简洁，相比第二种写法也强调了自身泛型函数的身份。

最后，还有一种写法，就是不定义概念，而是直接将其写在模板函数或模板类的定义上。例如，上述例子我们可以不定义概念 `Addable`，而是直接写在 `add` 函数的定义中：

```cpp
template<typename T>
T add(T a, T b) requires requires(T a, T b) { a + b; }  // 注意 requires 需要使用两次;
{ }
```

在你的要求集合比较短，并且只使用一次不想专门为其定义一个概念时，可以使用这种方法。同样 `requires`{:.language-cpp} 子句放在函数签名前后都可以，不过如果放在函数签名前，会把 Rouge 的染色干懵逼，导致 Rouge 误以为第二个 `requires`{:.language-cpp} 是函数名而渲染错误，所以在本帖中就不展示那种情况了。
